...
export default function RandomCities({favoriteCities, session}) {
      ...
      <div className={classes.cards}>
        {predefinedCities.map((city, index) => (
          <div key={index} className={classes.cardContainer}>
            <CityCard favoriteCities={favoriteCities} session ={session} ref={(el) => (cityCardRefs.current[index] = el)} />
          </div>
        ))}
      </div>
    </div>
